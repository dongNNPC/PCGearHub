...
app.controller("loadForm", function ($scope, $location, $http) {
	...
	$scope.edit = function () {
		...
		var url = `${host}/brands/${id}`;
		$http.get(url).then(resp => {
			$scope.brand = resp.data;
			console.log("Success", resp);
			console.log("Success", $scope.brand.admin);
			console.log("Success", $scope.brand.status);
	...
	$scope.validation = function () {
		var itemm = angular.copy($scope.brand);
	...
	$scope.update = function () {
		...
		var item = angular.copy($scope.brand);
		var url = `${host}/brands/${$scope.brand.id}`;
