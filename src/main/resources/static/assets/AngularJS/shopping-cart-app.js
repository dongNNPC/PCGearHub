...
app.controller("dangky", function ($scope, $http, $location) {
	...
	$scope.user = {
		...
		address: 'defaultAddress' // Giá trị mặc định cho trường địa chỉ
	};
	...
	$scope.create = function () {
		...
		if (!$scope.user.address) {
			$scope.user.address = 'null'; // Giá trị mặc định cho địa chỉ
		}

		var item = angular.copy($scope.user);
		var url = `${host}/users`;

		// Gán giá trị mặc định cho biến item.admin
		item.admin = false;
		...
		$http.post(url, item).then(resp => {
			...
			// Thực hiện các xử lý khác sau khi thêm thành công
		}).catch(error => {
			// Xử lý lỗi nếu cần
			console.log("Error", error);
		});
